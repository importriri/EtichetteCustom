...
public final class Disegno {
    ...
    public static synchronized String famiglia() {
        if (famiglia == null) {
            famiglia = "SansSerif";
            try {
                String[] presenti = GraphicsEnvironment.getLocalGraphicsEnvironment().getAvailableFontFamilyNames();
                for (int i = 0; i < PREFERITE.length && famiglia.equals("SansSerif"); i++) {
                    for (String p : presenti) {
                        if (p.equalsIgnoreCase(PREFERITE[i])) { famiglia = p; break; }
                    }
                }
            } catch (Throwable t) { famiglia = "SansSerif"; }
        }
        return famiglia;
    }
    ...
    public static void disegna(Graphics2D g, Etichetta eti, double mmPx,
        ...
        for (Elemento e : eti.elementi()) {
            ...
            try {
                ...
                switch (e.rotazione()) {
                    case 90: g2.translate(base.height * mmPx, 0); g2.rotate(Math.PI / 2); break;
                    case 180: g2.translate(base.width * mmPx, base.height * mmPx); g2.rotate(Math.PI); break;
                    case 270: g2.translate(0, base.width * mmPx); g2.rotate(3 * Math.PI / 2); break;
                    default: break;
                }
    ...
    private static void disegnaElemento(Graphics2D g, Etichetta eti, Elemento e,
        ...
        switch (e.tipo()) {
            case QR:
                /* QR: sempre il dato originale, mai la versione visuale. */
                disegnaQr(g, qr, eti.contenuto(e, copia), e.correzione(), 0, 0, e.larghezza() * mmPx);
                break;
            case BARCODE:
                disegnaBarcode(g, eti.contenuto(e, copia), 0, 0, e.larghezza() * mmPx, e.altezza() * mmPx);
                break;
            case LINEA:
                g.fillRect(0, 0, (int) Math.round(e.larghezza() * mmPx), Math.max(1, (int) Math.round(e.altezza() * mmPx)));
                break;
            default:
                disegnaScritta(g, eti, e, mmPx, copia);
                break;
        }
    }
    ...
    private static void disegnaScritta(Graphics2D g, Etichetta eti, Elemento e,
        ...
        String raw = eti.contenuto(e, copia);
        String testo = e.mostraSeparatori() ? raw : testoPulito(raw);
        Misuratore m = misuratore(g, mmPx);
        Testo.Esito esito = Testo.componi(testo, e.larghezza(), e.corpo(), e.massimoRighe(), e.grassetto(), m);

        Font f = font(esito.corpo() * mmPx, e.grassetto());
        g.setFont(f);
        FontMetrics fm = g.getFontMetrics();
        double interlinea = fm.getAscent() + fm.getDescent();
        double linea = fm.getAscent();
        double box = e.larghezza() * mmPx;
        for (String riga : esito.righe()) {
            int rw = fm.stringWidth(riga);
            double x = 0;
            if (e.allineamento() == 1) x = Math.max(0, (box - rw) / 2.0);
            else if (e.allineamento() == 2) x = Math.max(0, box - rw);
    ...
    /**
     * Nasconde soltanto i separatori dal testo umano. Il dato non viene mai
     * alterato e continua identico nel QR/barcode e nel file salvato.
     * I separatori diventano spazi utili al wrapping, poi vengono compattati.
     */
    private static String testoPulito(String raw) {
        ...
        for (int i = 0; i < raw.length(); i++) {
            char c = raw.charAt(i);
            if (c == '.' || c == '_' || c == '-' || c == '/' || c == ':' || c == ';' || c == ',') {
                if (b.length() > 0 && !spazio) { b.append(' '); spazio = true; }
            } else {
                b.append(c);
                spazio = false;
            }
    ...
    private static void disegnaQr(Graphics2D g, SorgenteQr qr, String contenuto,
        ...
        for (int r = 0; r < n; r++) {
            ...
            while (c < n) {
                if (m[r][c]) {
                    int larghi = 1;
                    while (c + larghi < n && m[r][c + larghi]) larghi++;
    ...
    private static void disegnaBarcode(Graphics2D g, String contenuto, double x, double y,
        ...
        try { tratti = Code128.tratti(contenuto); }
            ...
            g.drawRect((int) Math.round(x), (int) Math.round(y), (int) Math.round(larghezza), (int) Math.round(altezza));
            return;
        }
        int totale = 0;
        for (int t : tratti) totale += t;
    ...
    public static double moduloBarcodeMm(String contenuto, double larghezzaMm) {
        int totale = 0;
        for (int t : Code128.tratti(contenuto)) totale += t;
